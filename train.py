...
def train_Sanyo(model: nn.Module,
    ...
    for i, (train_batch, labels_batch) in enumerate(tqdm(train_loader, disable=args.tqdm)):
        ...
        x_dec = train_batch[:, (params.predict_start):, :params.dec_in].clone()
        x_dec[:, params.label_len:, 0] = 0
        x_mark_dec = train_batch[:, (params.predict_start):, params.dec_in:].clone()
        ...
        with amp.autocast(enabled=use_amp):
            mu_DNAR, sigma_DNAR, hidden_DNAR, indices = model.forward_DAR(x_enc, x_mark_enc, x_dec, x_mark_dec)
...
def train_and_evaluate_Sanyo(model: nn.Module,
    ...
    # print('lr: ',optimizer_NAR)
    # print('lr: ',optimizer_NAR.param_groups[0]['lr'])
