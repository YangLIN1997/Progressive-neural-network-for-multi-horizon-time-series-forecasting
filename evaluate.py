...
def evaluate(model,  loss_fn, test_loader, params, plot_num, sample=True, plot=True):
    ...
    with torch.no_grad():
        ...
        raw_metrics_DNAR = utils.init_metrics(sample=sample)
        raw_metrics_DAR = utils.init_metrics(sample=sample)
        ...
        for i, (test_batch, scale, mean, labels) in enumerate((test_loader)):
            # test_batch = test_batch.to(params.device)
            # batch_size = test_batch.shape[0]

            # scale = scale.to(params.device)
            # mean = mean.to(params.device)
            ...
            x_enc = test_batch[:, 0:params.predict_start, :params.dec_in].clone()
            x_mark_enc = test_batch[:, 0:params.predict_start, params.dec_in:].clone()
            x_dec = test_batch[:, (params.predict_start ):, :params.dec_in].clone()
            x_dec[:, params.label_len:, 0] = 0
            # print(x_dec[:, params.predict_start:, 0])
            x_mark_dec = test_batch[:, (params.predict_start ):, params.dec_in:].clone()


            # enc_out = model.encode(x_enc, x_mark_enc)
            mu, sigma, hidden, indices = model.test_DAR(x_enc, x_mark_enc, x_dec, x_mark_dec)
            # print(test_batch[0,0,3].item(),enc_out[0,0,0].item(),mu[0,0].item())
            ...
            # for i in range(7):
            #     mu[:,i*params.predict_steps:(i+1)*params.predict_steps],alpha[:,i*(params.predict_steps):(i+1)*(params.predict_steps)],sigma[:,i*params.predict_steps:(i+1)*params.predict_steps] \
            ...
            raw_metrics = utils.update_metrics(raw_metrics, input_mu, sample_mu, sample_j, sigma, labels,
            ...
            if plot == True and i == plot_batch:
                ...
                plot_eight_windows(params.plot_dir, plot_mu, label_plot, params.test_window, params.test_predict_start,
                                   plot_num, plot_metrics, sampling=sample)

            # plot_attention(params.plot_dir,plot_num,score[combined_sample])
            # plot_score = score[combined_sample].data.cpu().numpy()
            # plot_att(params.plot_dir,plot_score, plot_mu, label_plot, params.test_window, params.test_predict_start, plot_num, plot_metrics)
            ...
            # print(hidden[-1].shape,hidden_DNAR[-1].shape,hidden_DAR[-1].shape)

            with open(os.path.join(params.model_dir, 'results.npy'), 'wb') as f:
                np.save(f, labels.cpu())
                np.save(f, input_mu.cpu())
                np.save(f, sample_mu.cpu())
                np.save(f, sigma.cpu())
                np.save(f, hidden[-1].cpu())
                #
            if torch.cuda.is_available():
                measures = torch.zeros(10, )
                for i_speed in range(10):
                    start = torch.cuda.Event(enable_timing=True)
                    end = torch.cuda.Event(enable_timing=True)
                    start.record()
                    model.test_DAR(x_enc, x_mark_enc, x_dec, x_mark_dec)
                    end.record()
                    torch.cuda.synchronize()
                    measures[i_speed] = start.elapsed_time(end)
                # print('NAR mean:', torch.mean(measures), ' std:', torch.std(measures))
                logger.info('NAR mean:'+ str(round(torch.mean(measures).item(),1))+'  std:'+str(round(torch.std(measures).item(),1)))

    return summary_metric, summary_metric['test_loss']
