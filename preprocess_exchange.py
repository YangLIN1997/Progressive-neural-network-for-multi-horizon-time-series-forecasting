...
def prep_data(data, data_mean, data_scale, task='search_', name='train', data2=None):
    ...
    # print(n_id, (time_len - input_size) , stride_size)
...
def gen_covariates(times, num_covariates, n_id):
    ...
    for i in range(n_id):
        ...
        cov_age[data_start[i]:] = np.arange(times.shape[0] - data_start[i])
...
if __name__ == '__main__':
    ...
    # data_frame.set_index(pd.date_range("1990", freq="D", periods=data_frame.shape[0]))
    # date = pd.date_range("1990", freq="D",periods=10622)
    date = pd.date_range("1990", freq="D",periods=10622)
    data_frame = pd.DataFrame(data_frame.values, date[date.dayofweek < 5])
    # print(data_frame.shape)
    # print(data_frame)
    # print(data_frame[:6071]) # consistant with gluonts
    # print(pd.date_range("1990", freq="D", periods=data_frame.shape[0]))
