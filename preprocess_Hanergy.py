...
parser.add_argument('--L', default=1, type=int,help='Number of input period')
...
def prep_data(data, name='train'):
    ...
    if total_windows*stride_size<(time_len-input_size):
        total_windows+=1
    ...
    for i in range(total_windows):
        ...
        if (i+1)*stride_size<=(time_len-input_size):
            ...
            window_end = window_start + window_size-((i+1)*stride_size-(time_len-input_size))
            input_end = window_size-((i+1)*stride_size-(time_len-input_size))
            x_input[i, 1:input_end, 0] = data[window_start:window_end - 1, 0]
            x_input[i, :input_size, 1:] = data[window_start:forecast_start, 1:-4]
            x_input[i, input_size:input_end, 1:5] = data[forecast_start:window_end, -4:]
            x_input[i, input_size:input_end, 5:] = data[forecast_start:window_end, 5:-4]
            label[i, :input_end] = data[window_start:window_end, 0]
...
if __name__ == '__main__':
    ...
    # Split data
    train_start = 0
    train_end = data.shape[0] - 365*day_steps*3
    valid_start = data.shape[0] - 365*day_steps*3 - input_size
    valid_end = data.shape[0] - 365*day_steps*2
    test_start = data.shape[0] - 365*day_steps*2 - input_size
    test_end = data.shape[0] - 365*day_steps*1
    train_data = data[train_start:train_end,:].copy()
    valid_data = data[valid_start:valid_end,:].copy()
    test_data = data[test_start:test_end,:].copy()
    # Standardlize data
    st_scaler = StandardScaler()
    st_scaler.fit(train_data[:,np.array([0,1,2,3,4,8,9,10,11])].copy())
    train_data[:,np.array([0,1,2,3,4,8,9,10,11])] = st_scaler.transform(train_data[:,np.array([0,1,2,3,4,8,9,10,11])].copy())
    valid_data[:,np.array([0,1,2,3,4,8,9,10,11])] = st_scaler.transform(valid_data[:,np.array([0,1,2,3,4,8,9,10,11])].copy())
    test_data[:,np.array([0,1,2,3,4,8,9,10,11])] = st_scaler.transform(test_data[:,np.array([0,1,2,3,4,8,9,10,11])].copy())
    ...
    # Split data
    train_start = 0#14600
    train_end = data.shape[0] - 365*day_steps*2
    valid_start = data.shape[0] - 365*day_steps*2 - input_size
    valid_end = data.shape[0] - 365*day_steps*1
    test_start = data.shape[0] - 365*day_steps*1 - input_size
